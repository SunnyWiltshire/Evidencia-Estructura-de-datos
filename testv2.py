...
def reporte_prestamos_por_retornar(prestamos):
    if prestamos:
        ...
        if export_opcion == 1:
            ...
            export_excel_prestamos_retornar(prestamos, name_excel="Prestamos_por_retornar.xlsx")
        elif export_opcion == 3:
            export_csv_prestamos_retornar(prestamos, fecha_inicial, fecha_final)
            export_excel_prestamos_retornar(prestamos, name_excel="Prestamos_por_retornar.xlsx")
        elif export_opcion == 4:
...
def export_excel_prestamos_retornar(prestamos, name_excel="Prestamos_por_retornar.xlsx"):
# Crear el archivo Excel y agregar la hoja
    libro = openpyxl.Workbook()
    hoja = libro.active
    hoja.title = "Préstamos por retornar"
    hoja["A1"].font = Font(bold=True)
    hoja["B1"].font = Font(bold=True)
    hoja["C1"].font = Font(bold=True)
    hoja["D1"].font = Font(bold=True)
    hoja["E1"].font = Font(bold=True)
    # Leer el CSV
    with open('prestamos_bicicletas.csv', newline='', encoding='utf-8') as csvfile:
        csv_reader = csv.reader(csvfile)
        header = next(csv_reader)  # Encabezado
        hoja.append(header)  # Agregar el encabezado

        # Filtrar las filas donde "Retorno" es False y agregar al Excel
